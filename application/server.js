...
async function send_verification_email(email, verification_key) {
    ...
    // ...
    // mydestination = jrgroup.cse356.compas.cs.stonybrook.edu, jojrgroupey.cse356.compas.cs.stonybrook.edu, localhost.c>
    ...
    return new Promise((resolve, reject) => {
        ...
        let mailOptions = {
            from: 'jrgroup@cse356.compas.cs.stonybrook.edu',
            to: email,
...
app.use('/logout', async (req, res) => {
    ...
    if (req.session.login) {
        ...
        res.status(200).send({ status: "ERROR", message: 'Log out failed. Not previously logged in' });
    }
});
